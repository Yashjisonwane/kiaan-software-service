...
import { HRMFeatureCards } from './HRMFeatureCards';
import { HRMPricingSection, HRMPricingPlan } from './HRMPricingSection';
...
interface Props {
  ...
  description: string;
  features?: any[];
  pricingPlans?: HRMPricingPlan[];
  pricingSubtitle?: string;
}

export const HRMTemplate: React.FC<Props> = ({ title, description, features, pricingPlans, pricingSubtitle }) => {
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-10 bg-black/40 backdrop-blur-sm border-y border-white/5">
        <HRMFeatureCards features={features} />
      </div>

      {/* Bottom Half: Pricing Section */}
      <div className="relative z-10">
        <HRMPricingSection plans={pricingPlans} subtitle={pricingSubtitle} />
      </div>
    </div>
  );
};
