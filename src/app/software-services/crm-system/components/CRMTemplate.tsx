...
import { CRMFeatureCards } from './CRMFeatureCards';
import { CRMPricingSection, CRMPricingPlan } from './CRMPricingSection';
...
interface Props {
  ...
  description: string;
  features?: any[];
  pricingPlans?: CRMPricingPlan[];
  pricingSubtitle?: string;
}

export const CRMTemplate: React.FC<Props> = ({ title, description, features, pricingPlans, pricingSubtitle }) => {
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="relative z-10 bg-black/40 backdrop-blur-sm border-y border-white/5">
        <CRMFeatureCards features={features} />
      </div>

      {/* Bottom Half: Pricing Section */}
      <div className="relative z-10">
        <CRMPricingSection plans={pricingPlans} subtitle={pricingSubtitle} />
      </div>
    </div>
  );
};
